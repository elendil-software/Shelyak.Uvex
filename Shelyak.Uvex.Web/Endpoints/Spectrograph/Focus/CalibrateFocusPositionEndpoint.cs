...
public class CalibrateFocusPositionEndpoint : SpectrographWithValueEndpoint<float>
{
    private readonly IUsisDevice _usisDevice;

    public CalibrateFocusPositionEndpoint(IUsisDevice usisDevice, IServerTransactionIdProvider serverTransactionIdProvider, ILogger<CalibrateFocusPositionEndpoint> logger)
    ...
    {
        Put(DeviceNumberRoutePattern + ApiRoutes.CalibrateFocusPosition);
        base.Configure();
    }
    
    public override async Task HandleAsync(AlpacaRequestWithValue<float> request, CancellationToken ct)
    {
        UsisFunc = () => _usisDevice.CalibrateFocusPosition(request.Value);
