...
public record UpdateGratingAngleStepSizeCommand(float StepSize) : ICommand<Result<EmptyResult>>
{
    public class Handler : CommandHandlerBase<UpdateGratingAngleStepSizeCommand, EmptyResult>
    {
        ...
        protected override async Task<HttpResponseMessage> ExecuteHttpRequest(UpdateGratingAngleStepSizeCommand command, CancellationToken ct)
        {
            return await HttpClient.PutAsJsonAsync(UpdateGratingAngleStepSizeRequest.Route, new UpdateGratingAngleStepSizeRequest(command.StepSize), ct);
        }
