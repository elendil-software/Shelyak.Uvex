...
public record UpdateFocusStepSizeCommand(float StepSize) : ICommand<Result<EmptyResult>>
{
    public class Handler : CommandHandlerBase<UpdateFocusStepSizeCommand, EmptyResult>
    {
        ...
        protected override async Task<HttpResponseMessage> ExecuteHttpRequest(UpdateFocusStepSizeCommand command, CancellationToken ct)
        {
            return await HttpClient.PutAsJsonAsync(UpdateFocusStepSizeRequest.Route, new UpdateFocusStepSizeRequest(command.StepSize), ct);
        }
